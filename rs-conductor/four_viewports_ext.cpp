...
//  Camera generation (same keyframe data + interpolation as Hosting mode)
...
// Same 4-track keyframe data as renderstream.cpp Hosting mode
...
// CameraFn: evaluate keyframe tracks at time t, return pose for each camera
static std::vector<CamPose> EvalCameras(double t, int stream_w, int stream_h) {
    ...
    for (const auto& track : CameraTracks()) {
        ...
        poses.push_back(i0 == i1 ? keys[i0].pose : LerpPose(keys[i0].pose, keys[i1].pose,
...
static bool SendTick(SOCKET sock, double t, const std::vector<CamPose>& cameras,
                     int stream_w, int stream_h) {
    std::string json = "{\"t\":" + std::to_string(t) + ",\"cameras\":[";
    for (size_t i = 0; i < cameras.size(); ++i) {
        if (i > 0) json += ",";
        json += CameraToJson(cameras[i], stream_w, stream_h, static_cast<int>(i));
    }
    json += "]}\n";
    ...
    if (s_send_seq <= 5 || nl_count != 1) {
        fprintf(stderr, "[send] #%d len=%zu newlines=%zu tail='%s'",
    ...
    if (sent != static_cast<int>(json.size()))
        fprintf(stderr, "[send] #%d PARTIAL: sent=%d expected=%d\n", s_send_seq, sent, (int)json.size());
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < node_count; ++i) {
        ...
        //    ticks are flowing.
        fprintf(stderr, "[tick] starting loop at %.0f fps (with cameras)\n\n", kFps);
        double t = 0.0;
        ...
        while (true) {
            ...
            auto cameras = EvalCameras(t, cam_w, cam_h);
            if (!SendTick(tick_sock, t, cameras, cam_w, cam_h)) {
                fprintf(stderr, "[tick] send failed - UE may have exited\n");
                break;
            }

            if (++tick_count <= 3 || tick_count % 120 == 0)
                fprintf(stderr, "[tick] #%d t=%.3f cameras=%zu\n", tick_count, t, cameras.size());
